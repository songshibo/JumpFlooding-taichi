...
* ```init_step``` (tuple) the initial step length of JFA. Usually,``` init_step``` is $(\frac{width}{2}, \frac{height}{2})$. To reduce the computation time incurred by unnecessary JFA passes, ```init_step``` can be set to $(2^{\lceil\log{width} \rceil}, 2^{\lceil\log{height} \rceil})$.
...
* ```solve_cvt()``` will take $(2^{\lceil\log{width} \rceil}, 2^{\lceil\log{height} \rceil})$ as initial step size.
